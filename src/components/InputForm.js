...
function InputForm({ onSubmit }) {
  // State for each pollutant (matching the order of AirQualityInput class)
  const [pm2_5, setPm2_5] = useState(0); // PM2.5
  const [pm10, setPm10] = useState(0); // PM10
  const [no, setNo] = useState(0); // NO
  const [no2, setNo2] = useState(0); // NO2
  const [nox, setNox] = useState(0); // NOx
  const [nh3, setNh3] = useState(0); // NH3
  const [co, setCo] = useState(0); // CO
  const [so2, setSo2] = useState(0); // SO2
  const [o3, setO3] = useState(0); // O3
  const [benzene, setBenzene] = useState(0); // Benzene
  const [toluene, setToluene] = useState(0); // Toluene
  const [xylene, setXylene] = useState(0); // Xylene
